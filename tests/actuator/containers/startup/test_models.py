...
def test_startup_model_parsing():
    ...
    assert (
        timeline.start_time.isoformat()[:19] == "2025-04-23T10:23:43"
    )  # Skip milliseconds/microseconds
    ...
    # Test helper methods
    assert (
        abs(startup.get_total_startup_time() - 2.544) < 0.01
    )  # Approximately 2.544 seconds
    ...
    assert (
        abs(phases["spring.boot.application.environment-prepared"] - 0.1963843) < 0.0001
    )
...
def test_minimal_startup():
    ...
    json_data = {
        "timeline": {"startTime": "2025-04-23T00:00:00Z", "events": []},
        "springBootVersion": "3.4.4",
    }
