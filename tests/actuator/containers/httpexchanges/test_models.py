...
from src.actuator.containers.httpexchanges import (
    HttpExchanges, HttpExchange, HttpRequest, HttpResponse
)
...
def test_http_exchanges_model_parsing():
    ...
    assert exchange.timestamp.isoformat()[:19] == "2025-04-23T11:00:15"  # Skip microseconds
    ...
    assert request.headers["user-agent"] == ["Mozilla/5.0 (Windows NT 10.0; Win64; x64) Chrome/135.0.0.0"]
    ...
    # Test get_header helper method
    assert response.get_header("Content-Type") == "application/vnd.spring-boot.actuator.v3+json"
    assert response.get_header("content-type") == "application/vnd.spring-boot.actuator.v3+json"  # Case-insensitive
