...
def test_caches_model_parsing():
    ...
    assert other_cache.target == "com.github.benmanes.caffeine.cache.BoundedLocalCache$BoundedLocalManualCache"

    greeting_cache = cache_manager.get_cache("greetingCache")
    assert isinstance(greeting_cache, Cache)
    assert greeting_cache.target == "com.github.benmanes.caffeine.cache.BoundedLocalCache$BoundedLocalManualCache"
